...
没有真实 `data/sougou.log` 时，producer 会进入动态模拟模式，默认生成更接近真实热搜的新闻标题，例如：

```text
北京发布人工智能新政策，年轻人关注这些变化
新能源车进入落地关键期，多地试点释放新信号
华为回应大学生就业进展：将加快产品迭代
```

这样 `newscount` 不会固定在少量模板标题上，重启 producer 后也会继续产生新的新闻标题。

producer 还会默认混入少量格式差异，用来模拟真实日志里的干扰，例如空格、中文逗号、竖线、制表符、标题引号。Flink 作业会自动兼容这些格式。如果只想生成最规整的英文逗号 CSV，可在 `.env` 中设置：

```env
PRODUCER_NOISY_LOG_FORMATS=false
```
